/* This file is generated, do not edit! */
package pandas;
@:pythonImport("pandas.index") extern class Index {
	static public var UTC : Dynamic;
	static public var _SIZE_CUTOFF : Dynamic;
	static public var __doc__ : Dynamic;
	static public var __file__ : Dynamic;
	static public var __loader__ : Dynamic;
	static public var __name__ : Dynamic;
	static public var __package__ : Dynamic;
	static public var __spec__ : Dynamic;
	static public var __test__ : Dynamic;
	static public var _backfill_functions : Dynamic;
	static public var _pad_functions : Dynamic;
	static public function convert_scalar(args:haxe.extern.Rest<Dynamic>):Dynamic;
	static public function get_value_at(args:haxe.extern.Rest<Dynamic>):Dynamic;
	static public var have_pytz : Dynamic;
	static public function set_value_at(args:haxe.extern.Rest<Dynamic>):Dynamic;
}