...
@:pythonImport("matplotlib.colors", "PowerNorm") extern class PowerNorm {
	...
	**/
	@:native("__init__")
	public function ___init__(gamma:Dynamic, ?vmin:Dynamic, ?vmax:Dynamic, ?clip:Dynamic):Dynamic;
	/**
		If *vmin* or *vmax* is not given, they are initialized from the
		minimum and maximum value respectively of the first input
		processed.  That is, *__call__(A)* calls *autoscale_None(A)*.
		...
		*clip* = *False*.
	**/
	public function new(gamma:Dynamic, ?vmin:Dynamic, ?vmax:Dynamic, ?clip:Dynamic):Void;
	/**
		Return self<=value.
	**/
