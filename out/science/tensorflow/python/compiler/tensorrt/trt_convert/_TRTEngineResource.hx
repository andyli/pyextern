...
@:pythonImport("tensorflow.python.compiler.tensorrt.trt_convert", "_TRTEngineResource") extern class _TRTEngineResource {
	/**
	...
	**/
	@:native("__init__")
	public function ___init__(resource_name:Dynamic, filename:Dynamic, maximum_cached_engines:Dynamic, ?device:Dynamic):Dynamic;
	/**
		Initialize the `TrackableResource`.
		...
		Args:
		  device: A string indicating a required placement for this resource,
		    ...
		    should be blank unless the resource only makes sense on one device.
	**/
	public function new(resource_name:Dynamic, filename:Dynamic, maximum_cached_engines:Dynamic, ?device:Dynamic):Void;
	/**
