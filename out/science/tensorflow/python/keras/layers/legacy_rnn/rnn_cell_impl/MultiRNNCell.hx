...
@:pythonImport("tensorflow.python.keras.layers.legacy_rnn.rnn_cell_impl", "MultiRNNCell") extern class MultiRNNCell {
	...
	**/
	@:native("__init__")
	public function ___init__(cells:Dynamic, ?state_is_tuple:Dynamic):Dynamic;
	/**
		Create a RNN cell composed sequentially of a number of RNNCells.
		...
		Raises:
		  ValueError: if cells is empty (not allowed), or at least one of the cells
		    returns a state tuple but the flag `state_is_tuple` is `False`.
	**/
	public function new(cells:Dynamic, ?state_is_tuple:Dynamic):Void;
	/**
