...
@:pythonImport("tensorflow.python.ops.resource_variable_ops", "VariableSpec") extern class VariableSpec {
	/**
	...
	**/
	@:native("__init__")
	public function ___init__(shape:Dynamic, ?dtype:Dynamic, ?name:Dynamic, ?trainable:Dynamic):Dynamic;
	/**
		Creates a TensorSpec.
		...
		Raises:
		  TypeError: If shape is not convertible to a `tf.TensorShape`, or dtype is
		    not convertible to a `tf.DType`.
	**/
	public function new(shape:Dynamic, ?dtype:Dynamic, ?name:Dynamic, ?trainable:Dynamic):Void;
	/**
