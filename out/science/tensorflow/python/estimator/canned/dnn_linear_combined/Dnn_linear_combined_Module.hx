	...
	/**
		...
		Raises:
		  ValueError: If both `linear_feature_columns` and `dnn_features_columns`
		    ...
		    or features has the wrong type.
	**/
	static public function _dnn_linear_combined_model_fn_v2(features:Dynamic, labels:Dynamic, mode:Dynamic, head:Dynamic, ?linear_feature_columns:Dynamic, ?linear_optimizer:Dynamic, ?dnn_feature_columns:Dynamic, ?dnn_optimizer:Dynamic, ?dnn_hidden_units:Dynamic, ?dnn_activation_fn:Dynamic, ?dnn_dropout:Dynamic, ?config:Dynamic, ?batch_norm:Dynamic, ?linear_sparse_combiner:Dynamic, ?loss_reduction:Dynamic):Dynamic;
	/**
