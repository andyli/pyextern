	...
	/**
		...
		Raises:
		  ValueError: If elements contains an element that isn't a tf.Tensor or
		    tf.Operation.
	**/
	static public function _create_graph(?input_graph:Dynamic, ?is_training:Dynamic, ?weight_bits:Dynamic, ?activation_bits:Dynamic, ?symmetric:Dynamic, ?quant_delay:Dynamic, ?freeze_bn_delay:Dynamic, ?scope:Dynamic):Dynamic;
