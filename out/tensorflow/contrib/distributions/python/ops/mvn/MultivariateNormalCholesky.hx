...
@:pythonImport("tensorflow.contrib.distributions.python.ops.mvn", "MultivariateNormalCholesky") extern class MultivariateNormalCholesky {
	...
	**/
	@:native("__init__")
	public function ___init__(mu:Dynamic, chol:Dynamic, ?validate_args:Dynamic, ?allow_nan_stats:Dynamic, ?name:Dynamic):Dynamic;
	/**
		Multivariate Normal distributions on `R^k`.
		...
		Raises:
		  TypeError: If `mu` and `chol` are different dtypes.
	**/
	public function new(mu:Dynamic, chol:Dynamic, ?validate_args:Dynamic, ?allow_nan_stats:Dynamic, ?name:Dynamic):Void;
	/**
		Return self<=value.
	**/
