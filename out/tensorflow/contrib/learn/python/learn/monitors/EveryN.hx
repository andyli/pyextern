...
@:pythonImport("tensorflow.contrib.learn.python.learn.monitors", "EveryN") extern class EveryN {
	...
	**/
	@:native("__init__")
	public function ___init__(?every_n_steps:Dynamic, ?first_n_steps:Dynamic):Dynamic;
	/**
		Initializes an `EveryN` monitor.
		...
		Args:
		  ...
		  first_n_steps: `int`, specifying the number of initial steps during
		    ...
		    of `every_n_steps`. Note that this value is relative to the global step
	**/
	public function new(?every_n_steps:Dynamic, ?first_n_steps:Dynamic):Void;
	/**
		Return self<=value.
	**/
