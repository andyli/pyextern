...
package tensorflow.contrib.estimator.python.estimator.dnn_linear_combined;
@:pythonImport("tensorflow.contrib.estimator.python.estimator.dnn_linear_combined", "DNNLinearCombinedEstimator") extern class DNNLinearCombinedEstimator {
	...
	/**
		...
		Raises:
		  ValueError: If both linear_feature_columns and dnn_features_columns are
		    empty at the same time.
	**/
	public function new(head:Dynamic, ?model_dir:Dynamic, ?linear_feature_columns:Dynamic, ?linear_optimizer:Dynamic, ?dnn_feature_columns:Dynamic, ?dnn_optimizer:Dynamic, ?dnn_hidden_units:Dynamic, ?dnn_activation_fn:Dynamic, ?dnn_dropout:Dynamic, ?input_layer_partitioner:Dynamic, ?config:Dynamic, ?linear_sparse_combiner:Dynamic):Void;
	/**
