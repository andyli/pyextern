...
@:pythonImport("tensorflow.python.estimator.estimator", "Estimator") extern class Estimator {
	...
	**/
	@:native("__init__")
	public function ___init__(model_fn:Dynamic, ?model_dir:Dynamic, ?config:Dynamic, ?params:Dynamic, ?warm_start_from:Dynamic):Dynamic;
	/**
		Constructs an `Estimator` instance.
		...
		Raises:
		  ...
		  ValueError: if this is called via a subclass and if that class overrides
		    a member of `Estimator`.
	**/
	public function new(model_fn:Dynamic, ?model_dir:Dynamic, ?config:Dynamic, ?params:Dynamic, ?warm_start_from:Dynamic):Void;
	/**
