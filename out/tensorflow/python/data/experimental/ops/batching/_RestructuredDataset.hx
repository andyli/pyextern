...
@:pythonImport("tensorflow.python.data.experimental.ops.batching", "_RestructuredDataset") extern class _RestructuredDataset {
	/**
	...
	**/
	@:native("__init__")
	public function ___init__(dataset:Dynamic, output_types:Dynamic, ?output_shapes:Dynamic, ?output_classes:Dynamic, ?allow_unsafe_cast:Dynamic):Dynamic;
	/**
		Creates a new dataset with the given output types and shapes.
		...
		* Each shape in `dataset.output_shapes` must be compatible with each shape
		...
		Raises:
		  ValueError: If either `output_types` or `output_shapes` is not compatible
		    with the structure of `dataset`.
	**/
	public function new(dataset:Dynamic, output_types:Dynamic, ?output_shapes:Dynamic, ?output_classes:Dynamic, ?allow_unsafe_cast:Dynamic):Void;
	/**
