...
@:pythonImport("torch.distributed.optim.optimizer", "DistributedOptimizer") extern class DistributedOptimizer {
	...
	**/
	@:native("__init__")
	public function ___init__(optimizer_class:Dynamic, params_rref:Dynamic, ?args:python.VarArgs<Dynamic>, ?kwargs:python.KwArgs<Dynamic>):Dynamic;
	/**
		Initialize self.  See help(type(self)) for accurate signature.
	**/
	public function new(optimizer_class:Dynamic, params_rref:Dynamic, ?args:python.VarArgs<Dynamic>, ?kwargs:python.KwArgs<Dynamic>):Void;
	/**
	...
	/**
		...
		Args:
		    context_id: the autograd context id for which we should run the
		        optimizer step.
	**/
	public function step(context_id:Dynamic):Dynamic;
}