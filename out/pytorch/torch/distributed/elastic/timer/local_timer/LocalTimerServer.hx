...
@:pythonImport("torch.distributed.elastic.timer.local_timer", "LocalTimerServer") extern class LocalTimerServer {
	...
	**/
	@:native("__init__")
	public function ___init__(mp_queue:Dynamic, ?max_interval:Dynamic, ?daemon:Dynamic):Dynamic;
	/**
		:param request_queue: Consumer ``RequestQueue``
		:param max_interval: max time (in seconds) to wait
		                     for an item in the request_queue
		:param daemon: whether to run the watchdog thread as a daemon
	**/
	public function new(mp_queue:Dynamic, ?max_interval:Dynamic, ?daemon:Dynamic):Void;
	/**
